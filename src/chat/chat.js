...
export default class Chat extends Component {
    ...
    componentDidMount() {
        ...
        this.socket.on('connect', () => {
            this.socket.emit('register', {chatId: this.props.chatId, userId: this.props.userId, webName: this.props.web, leftString: this.props.conf.leftString});
        });
        
        //this.socket.on(this.props.chatId, this.incomingMessage);
        this.socket.on(this.props.userId + '-' + this.props.web, this.incomingMessage);
        
        var chatIdList = this.props.chatId.split(",");
        for (var i = 0; i < chatIdList.length; i++){
            var id = chatIdList[i];
            this.socket.on(id, this.incomingMessage);
            this.socket.on(id + '-' + this.props.userId + '-' + this.props.web, this.incomingMessage);
        };
